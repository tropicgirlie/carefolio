...
export function IndexDirectory() { return <Layout><section className="cf-page-head"><div className="cf-frame"><p className="cf-eyebrow">Carefolio Index™</p><h1>Company evidence directory</h1><p>Sorted by calculated Care Score. Filter by the evidence that matters to you, then inspect every source in the record.</p></div></section><DirectoryBlock title="All public evidence files" intro={`A ${scoredIndexRecords.length}-company research snapshot from the Carefolio and RemoteShe evidence system. No score is estimated. Where a policy is unknown, it earns zero and does not appear as a positive filter result.`} /></Layout>; }
